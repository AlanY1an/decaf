...
RELEASE_NOTES="${DECAF_RELEASE_NOTES:-$REPO_ROOT/docs/releases/$VERSION.md}"
SPARKLE_ACCOUNT="${DECAF_SPARKLE_ACCOUNT:-io.github.alany1an.decaf}"
...
step "Resolve pinned Sparkle and validate the update signing key"
xcodebuild -resolvePackageDependencies -project "$PROJECT" -scheme "$SCHEME" \
    -clonedSourcePackagesDirPath "$SOURCE_PACKAGES" >/dev/null
SPARKLE_BIN="${DECAF_SPARKLE_BIN:-$SOURCE_PACKAGES/artifacts/sparkle/Sparkle/bin}"
if [ "$DRY_RUN" -eq 0 ]; then
    ...
    [ "$PUBLIC_KEY" = "$("$SPARKLE_BIN/generate_keys" --account "$SPARKLE_ACCOUNT" -p)" ] \
        || die "Sparkle signing key does not match SUPublicEDKey. Do not regenerate a shipped key."
    ok "Sparkle signing key matches the app (private key stays in Keychain)"
fi
...
# Feed and archive signatures are produced after notarization/stapling. Never
# publish a placeholder signature or generate a distributable feed in a dry run.
printf '%s  %s\n' "$DMG_SHA256" "$DMG_NAME" > "$OUT_DIR/$DMG_NAME.sha256"
if [ "$DRY_RUN" -eq 1 ]; then
    skip "dry run — no signed update feed generated"
else
    DECAF_SPARKLE_BIN="$SPARKLE_BIN" DECAF_SPARKLE_ACCOUNT="$SPARKLE_ACCOUNT" \
        ./Scripts/generate-appcast.sh "$DMG_PATH" "$EXPORTED_APP" "$RELEASE_NOTES" "$OUT_DIR"
    ok "appcast and final DMG signatures verified"
fi
...
if [ "$DRY_RUN" -eq 1 ]; then
    ...
    printf '  3. gh release create %s "%s" "%s" "%s" --notes-file "%s"\n' \
        "$TAG" "$DMG_PATH" "$APPCAST_PATH" "$OUT_DIR/$DMG_NAME.sha256" "$RELEASE_NOTES"

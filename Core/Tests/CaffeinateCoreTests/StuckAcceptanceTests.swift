...
@Suite struct StuckTruePositiveTests {
    ...
    @Test func theHardwareScenarioIsBoundedReportedAndKept() {
        ...
        #expect(downgrades.first?.cwd == "/Users/tester/Project/Caffeinate")
        ...
        #expect(notice.body.contains("Claude Code · Caffeinate"))
    ...
    @Test func theHoldReachesTheEngineAndIsThenReleased() async {
        ...
        let root = CompositionRoot(
            settings: SettingsStore(
                defaults: UserDefaults(suiteName: "dev.caffeinate.tests.stuck.\(UUID().uuidString)")!
            ),
            asserter: asserter,
            displaySleeper: FakeDisplaySleeper(),
            socketPath: NSTemporaryDirectory() + "caffeinate-stuck-\(UUID().uuidString).sock"
        )
...
@Suite struct StuckAdversarialTests {
    ...
    @Test func aHeartbeatFloodCostsAtMostOneWrite() async {
        ...
        let temp = FileManager.default.temporaryDirectory
            .appendingPathComponent("caffeinate-flood-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func aSnapshotFromAPreviousBootIsNotRestored() throws {
        let temp = FileManager.default.temporaryDirectory
            .appendingPathComponent("caffeinate-boot-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func aDowngradeLeavesAManualHoldAlone() async {
        ...
        let root = CompositionRoot(
            settings: SettingsStore(
                defaults: UserDefaults(suiteName: "dev.caffeinate.tests.manual.\(UUID().uuidString)")!
            ),
            asserter: asserter,
            displaySleeper: FakeDisplaySleeper(),
            socketPath: NSTemporaryDirectory() + "caffeinate-manual-\(UUID().uuidString).sock"
        )

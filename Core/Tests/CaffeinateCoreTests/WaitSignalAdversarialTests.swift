...
private final class AdversarialTranscript {
    ...
    init(sessionID: String = "s-loop") {
        root = FileManager.default.temporaryDirectory
            .appendingPathComponent("caffeinate-adv-\(UUID().uuidString)", isDirectory: true)
        let directory = root
    ...
    @Suite struct TheCapOutlivesTheParser {
        ...
        @Test func aWaitOnlyHoldIsSuspendedByTheBatteryGate() async {
            ...
            let defaults = UserDefaults(
                suiteName: "dev.caffeinate.tests.adversarial.\(UUID().uuidString)"
            )!
            ...
            let root = CompositionRoot(
                ...
                socketPath: NSTemporaryDirectory() + "caffeinate-adv-\(UUID().uuidString).sock"
            )

...
- Match the native title bar to the Home and Settings backgrounds, including the
  sidebar. Open windows follow light/dark appearance changes without reopening.
- Use a consistent native title-bar background in update options, onboarding and
  custom keep-awake dialogs. Window controls and dragging remain native.

## [0.3.1] - 2026-09-11

### Added

- **Check for Updates…** in the cup menu, application menu and General settings.
  Sparkle 2.9.6 handles download, signature validation, installation and relaunch.
  Checks start only when requested; background checks and system profiling are off.
- Local Debug builds explain that public updates are unavailable and do not
  request the release feed. This prevents an unhelpful error before the first
  updater-enabled release is published.
- Signed update feeds and archive signatures in the release pipeline, backed by
  a maintainer Keychain key. The published feed and DMG are verified before upload.
- Compact update options keep manual download and Homebrew behind a disclosure.
  Versions 0.3.0 and earlier need one manual update before the new updater becomes
  available.

// SecondLaunchTests — what happens when a second copy of Decaf is
// launched (plan 04 step 1, plan 02 §1.4 control extension).
//
// The behaviour being pinned replaced an alert that said "Decaf is
...
private struct SocketSandbox {
    ...
    init() throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("decaf-sl-\(UUID().uuidString.prefix(8))")
...
struct SecondLaunchTests {
    ...
    @Test func anInstanceThatNeverInstalledAControlHandlerRefusesInsteadOfHanging() throws {
        ...
        // No controlHandler: the bare pipe (decaf-smoke, older builds).
        let server = HookSocketServer(socketPath: sandbox.path)
        try server.start()
        defer { server.stop() }
...
/// Writes one framed line and closes, the way decaf-bridge does.

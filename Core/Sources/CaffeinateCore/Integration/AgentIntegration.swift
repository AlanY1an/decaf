...
public enum IntegrationError: Error, Equatable {
    /// The copy source `Caffeinate.app/Contents/Helpers/caff-bridge` is missing.
    case bundledBridgeMissing(path: String)
}

/// Well-known absolute paths derived from a home directory (injected so tests
/// can point everything at a sandbox). App Support dir name is "Caffeinate";
...
public struct IntegrationPaths: Equatable, Sendable {
    ...
    public var appSupportDirectory: String { homeDirectory + "/Library/Application Support/Caffeinate" }
    public var binDirectory: String { appSupportDirectory + "/bin" }
    public var bridgeBinary: String { binDirectory + "/caff-bridge" }

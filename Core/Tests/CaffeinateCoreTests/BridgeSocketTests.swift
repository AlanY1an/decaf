// BridgeSocketTests — end-to-end integration of caff-bridge × HookSocketServer
// (plan 02 §1.3/§1.4 + step 8; plan 06 §5 "bridge" acceptance row).
//
// Spawns the actually-built caff-bridge executable with recorded hook fixtures
// on stdin (Tests/CaffeinateCoreTests/Fixtures/, plan 02 step 1) against a
...
/// builds the caff-bridge executable product into the same directory.
private let productsDirectory: URL = Bundle(for: BridgeSocketTestsToken.self)
    .bundleURL
    .deletingLastPathComponent()

private let bridgeBinaryURL = productsDirectory.appendingPathComponent("caff-bridge")
...
private func requireBridgeBinary() throws {
    try #require(
        ...
        "caff-bridge not built at \(bridgeBinaryURL.path) — run swift build first"
    )
}

// MARK: - Spawning caff-bridge
...
/// Runs the built caff-bridge with the given stdin/argv against `socketPath`
/// (injected via the CAFF_BRIDGE_SOCKET test seam) and waits for exit.
@discardableResult
private func runBridge(
    ...
    environment["CAFF_BRIDGE_SOCKET"] = socketPath
    ...
    if process.isRunning {
        Issue.record("caff-bridge did not exit within 5 s — watchdog broken")
        process.terminate()
    }
...
private final class ServerHarness {
    ...
    init(start: Bool = true) throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("caff-bst-\(UUID().uuidString.prefix(8))")

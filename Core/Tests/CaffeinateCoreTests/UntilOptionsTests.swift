...
@Suite @MainActor struct HoldUntilTests {
    ...
    private func makeRoot() -> (root: CompositionRoot, fake: FakePowerAsserter) {
        let defaults = UserDefaults(suiteName: "dev.caffeinate.tests.until.\(UUID().uuidString)")!
        ...
        let root = CompositionRoot(
            ...
            asserter: fake,
            socketPath: NSTemporaryDirectory() + "caffeinate-until-test-\(UUID().uuidString).sock"
        )
        return (root, fake)
    }

...
private struct Harness {
    ...
    let bundledBridgePath = "/Applications/Decaf.app/Contents/Helpers/decaf-bridge"
...
@Suite struct IntegrationTests {
        ...
        @Test func commandTemplateIsQuotedHomeRelativeWithArgvMatchers() {
            ...
            #expect(
                ClaudeSettingsEditor.quotedBridgeCommand
                    == "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\""
            )
            ...
            #expect(commands == [
                "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\" permission_prompt",
                "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\" idle_prompt",
            ])
        }
              ...
              "hooks": {
                "Stop": [
                  { "hooks": [
                      ...
                      { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\"", "timeout": 5 }
                  ] }
                ]
              }
            }
    ...
    @Suite struct ConfigFileEditing {
        ...
        @Test func defaultFileSystemWritesAtomicallyWithoutLeftovers() throws {
            ...
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("decaf-tests-\(UUID().uuidString)")
    ...
    @Suite struct ClaudeCodeIntegrationFlow {
        ...
        @Test func plannedChangesListBothFilesWithPreviews() throws {
            ...
            #expect(settingsPreview.contains("Application Support/Decaf"))
    ...
    @Suite struct HookSetUpgrade {
        ...
        /// Exactly what a pre-heartbeat Decaf wrote, with the user's own
        ...
        static let legacySettingsJSON = #"""
        {
          "model": "opus",
          "hooks": {
            "SessionStart": [ { "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\"", "timeout": 5 } ] } ],
            "UserPromptSubmit": [ { "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\"", "timeout": 5 } ] } ],
            "Stop": [
              { "hooks": [ { "type": "command", "command": "echo user-stop", "timeout": 30 } ] },
              { "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\"", "timeout": 5 } ] }
            ],
            "StopFailure": [ { "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\"", "timeout": 5 } ] } ],
            "SessionEnd": [ { "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\"", "timeout": 5 } ] } ],
            "Notification": [
              { "matcher": "permission_prompt", "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\" permission_prompt", "timeout": 5 } ] },
              { "matcher": "idle_prompt", "hooks": [ { "type": "command", "command": "\"$HOME/Library/Application Support/Decaf/bin/decaf-bridge\" idle_prompt", "timeout": 5 } ] }
            ],
            "PreToolUse": [
        ...
        /// What `legacySettingsJSON` reduces to once every Decaf entry —

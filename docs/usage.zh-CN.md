...
2. 打开咖啡杯菜单 → **Today: … tokens…**，查看每日统计。新安装默认点击打开菜单；老用户保留左键切换保活、右键打开菜单的习惯。可在 **Settings → General → Menu Bar** 中选择。
...
从咖啡杯菜单打开 **Your brew…**（**⇧⌘P**），查看个人页：90 天有记录的活动、月度用量和 Claude Code/Codex 的 token 占比。用月份旁的左右箭头回看历史；选择过去的月份时，活动图截止于当月最后一天，点 **This month** 返回本月。到 **Settings → Profile** 或页面的 **Personalize** 修改可选昵称和咖啡图标。**Make a monthly card** 会先预览所选月份实际导出的图片，再由你复制或保存；卡片可选择 **System / Light / Dark** 配色，保存文件名带月份。默认隐藏 token 数字及其强度信息；勾选 **Show token totals** 才会加入。空白日期可能是缺失的历史，同一天使用两个工具只算一个活跃日。

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/profile-dark.png">
  <img src="assets/profile-light.png" alt="Your brew 个人页：昵称、咖啡印章、本月用量、90 天活动格子和月度卡片入口" width="520">
</picture>

<sub>Your brew：原生界面，昵称与数据均为示例。</sub>
...
| Your brew 个人页 | — | 本地昵称与图标、90 天活动和月度分享卡 |
| 复制每日 / 每月小票 | — | 本机生成 PNG |
| 手动保持、定时与电量保护 | 支持 | 支持 |
...
- 昵称、图标和分享偏好仅保存在本地，不会自动读取系统账号身份。在 Settings → Profile 清空昵称即可恢复通用卡片。

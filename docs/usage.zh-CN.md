...
**0.3.1 及后续版本：** 杯子菜单或 **Settings → General → Check for Updates…**
可以在应用内检查、下载、安装并重启，设置和用量记录保留。打开设置本身不会联网。
0.3.0 及更早版本需要先按下面的方式手动升级一次。Homebrew 用户也可以继续使用
原来的命令，**Update options…** 中保留了这些说明。[更新实现说明](updating.md)。
...
- **检测和统计留在本机。** 从 0.3.1 起，主动检查或下载更新会连接 GitHub 及其下载 CDN；没有后台检查、分析追踪、系统信息采集或对话上传。点击发布页链接会打开浏览器。
